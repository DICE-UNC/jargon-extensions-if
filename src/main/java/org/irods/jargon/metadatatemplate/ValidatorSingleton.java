...
public final class ValidatorSingleton {
	...
	public ValidationReturnEnum validate(IRODSAccount irodsAccount, IRODSAccessObjectFactory irodsAccessObjectFactory,
		...
		if (me.getValidationStyle() == ValidationStyleEnum.IN_RANGE
			...
			/*
			 * IN_RANGE validation only makes sense for well-ordered values (i.e. numbers
			 * and dates)
			 * 
			 * Need to check type, and whether endpoints are valid, before validating.
			 */
		...
		if (me.getValidationStyle() == ValidationStyleEnum.FOLLOW_REF) {
			...
			if (me.getType() == ElementTypeEnum.REF_IRODS_QUERY) {
				...
				/*
				 * FOLLOW_REF is only applicable for reference-type MetadataElements. If used
				 * with a raw-type element, return NOT_VALIDATED
				 */
				return ValidationReturnEnum.NOT_VALIDATED;
			}
		}
